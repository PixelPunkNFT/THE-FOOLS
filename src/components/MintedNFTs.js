...
const MintedNFTs = () => {
  ...
  const fetchMetadataInBatch = useCallback(async (uris) => {
    try {
      console.log("Fetching metadata for URIs:", uris);
      ...
      return uris.map(() => PLACEHOLDER_IMAGE);
    }
  }, [convertIpfsToHttp]);

  const processBatch = useCallback(async (start, end, totalSupply) => {
    const batchPromises = [];
  ...
  }, [blockchain.smartContract, fetchMetadataInBatch]);
  ...
  useEffect(() => {
      ...
      if (blockchain.smartContract) {
        try {
          ...
          for (let i = 1; i <= totalSupply; i += batchSize) {
            ...
            console.log(`Processing batch from ${i} to ${end}`);
    ...
    fetchNFTs();
  }, [blockchain.smartContract, processBatch]);
